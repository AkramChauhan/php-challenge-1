...
if(isset($_REQUEST['string']) && $_REQUEST['string']!=""){
  $table_where .= " (username like '%".$_REQUEST['string']."%')
    OR (name like '%".$_REQUEST['string']."%')
    OR (uid like '%".$_REQUEST['string']."%')  
  ";
...
$table_rr = $db->get($table,"*",$table_where,"id ASC limit $page_position, $item_per_page");
?>
<table id="example1" class="table table-sm table-bordered table-striped mb-0">
    <thead>
        <tr>
            <th>No.</th>
            <th>Name</th>
    ...
    if($table_rr->rowCount()>0){
        ...
        <tr>
            ...
            <td><?php echo stripslashes($table_d['name']); ?></td>
            <td><?php echo stripslashes($table_d['username']); ?></td>
            <td><?php echo stripslashes($table_d['uid']); ?></td>
            <td>
